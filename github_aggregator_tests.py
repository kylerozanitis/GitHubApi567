...
class TestGitHubAggregator(unittest.TestCase):
    ...
    def test_gather_repos(self):
        @patch('gather_repos.requests.get')
        def test_gather_repos(mock_get):
            mock_get.return_value = ['DSA', 'expressionevaluator', 'gedcom-parser', 'GitHubApi567', 'hello-world', 'Personal-Website', 'school-data-management-system', 'SSW-567', 'Triangle567']
            response = gather_repos('kylerozanitis')
            self.assertEqual(response, ['DSA', 'expressionevaluator', 'gedcom-parser', 'GitHubApi567', 'hello-world', 'Personal-Website', 'school-data-management-system', 'SSW-567', 'Triangle567'])
    ...
    def test_gather_commits(self):
        @patch('gather_commits.requests.get')
        def test_gather_commits(mock_get):
            mock_get.return_value = ('Triangle567', 11)
            response = gather_commits('kylerozanitis', 'Triangle567')
            self.assertEqual(response, ('Triangle567', 11))
    ...
    def test_gather_github_info(self):
        @patch('gather_github_info.requests.get')
        def test_gather_github_info(mock_get):
            mock_get.return_value = [('DSA', 9), ('expressionevaluator', 6), ('gedcom-parser', 30), ('GitHubApi567', 16), ('hello-world', 3), ('Personal-Website', 11), ('school-data-management-system', 7), ('SSW-567', 9), ('Triangle567', 11)]
            response = gather_github_info('kylerozanitis')
            self.assertEqual(response, [('DSA', 9), ('expressionevaluator', 6), ('gedcom-parser', 30), ('GitHubApi567', 16), ('hello-world', 3), ('Personal-Website', 11), ('school-data-management-system', 7), ('SSW-567', 9), ('Triangle567', 11)])
